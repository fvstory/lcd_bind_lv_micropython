...
void lcd_bus_flush(void *_disp_drv, const void *_area, void *_color_p)
{
    lv_disp_drv_t *disp_drv = _disp_drv;
    const lv_area_t *area = _area;
    lv_color_t *color_p = _color_p;
    ...
    mp_lcd_i80_obj_t *bus_obj = MP_OBJ_TO_PTR(mp_obj_dict_get(disp_drv->user_data, MP_OBJ_NEW_QSTR(MP_QSTR_bus_obj)));
    bus_obj->disp_drv_obj = disp_drv;
    ...
    if (mp_obj_is_type(bus_obj, &mp_lcd_i80_type)) {
        hal_lcd_i80_tx_param(bus_obj, 0x2A, (uint8_t[]) {
            ((x1 >> 8) & 0xFF),
            (x1 & 0xFF),
            ((x2 >> 8) & 0xFF),
            (x2 & 0xFF),
        }, 4);
        hal_lcd_i80_tx_param(bus_obj, 0x2B, (uint8_t[]) {
            ((y1 >> 8) & 0xFF),
            (y1 & 0xFF),
            ((y2 >> 8) & 0xFF),
            (y2 & 0xFF),
        ...
        DEBUG_printf("x1: %d, x2: %d, y1: %d, y2: %d ,buf_len: %u\n", x1, x2, y1, y2, len);
        hal_lcd_i80_tx_color(bus_obj, 0x2C, color_p, len);
    }
    //gc_collect();
}
